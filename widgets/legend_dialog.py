...
class GenerateLegendDialog(QDialog):
    ...
    def saveFile(self):
        ...
        try:
            with open(self.savedLegendPath, "w", encoding="utf-8") as f:
                for key in keys:
    ...
    def addLabel(self):
        ...
        if self.labelList.count()>0:
            items = self.labelList.findItems(label, Qt.MatchExactly)
            if items:
                item = items[0]
    ...
    def delete_selected_rows(self):
        ...
        for idx in selectedItems:
            if self.table.item(idx.row(),1):
                ...
                if len(self.labels)>0 and self.legend_data[id] in self.labels:
