...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setDirty(self):
        ...
        if self.actions.saveAuto.isChecked():
            ...
            self.buffer[self.imagePath]["dirty"] = self.saveLabels(label_file, self.imagePath)!=True
            return
        self.dirty = True
        self.bufferCurrentStatus()
    ...
    def runYoloVid(self):
        ...
        for i, image in enumerate(images):
            ...
            self.buffer[image]["dirty"] = True
            if self.actions.saveAuto.isChecked():
                if not self.saveLabels(self.getOutputLabelFile(image), image):
                    self.actions.save.setEnabled(True)
                    break
    ...
    def runYoloTrack(self):
        ...
        self.actions.undo.setEnabled(True)
        self.actions.saveAllAs.setEnabled(True)
        self.actions.trajectory.setEnabled(True)
        if self.actions.saveAuto.isChecked():
            self.actions.saveAll.setEnabled(False)
        else:
            self.actions.saveAll.setEnabled(True)
    ...
    def fillGapVideo(self):
        ...
        self.actions.saveAll.setEnabled(not self.actions.saveAuto.isChecked())
        self.actions.saveAllAs.setEnabled(True)
