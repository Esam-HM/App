...
## updates target_dict values from new_dict.
## if values in target and new_dict are dictionary. update them recursively.
## validate_item : takes function that check for valid values for specific keys.
def update_dict(target_dict, new_dict, validate_item=None):
    for key, value in new_dict.items():
        if validate_item:
            validate_item(key, value)
        if key not in target_dict: ## key not found in target_dict.
...
## create new config file with default values (if not exist).
