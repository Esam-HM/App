...
## collections: Provide special data types.
## threading: Provide ability to use threads in the program.
## imgviz:
## onnxruntime: Used for cross platform programs which need to work on different machines.
## Change any model format (pytorch,tensorflow,...) to onnx format.
## provide high performance working.
## skimage: image processing library. filters, transform, feature extraction..
...
class EfficientSam:
    ...
    ## get image embeddings if cached. else, compute embeddings.
    def set_image(self, image: np.ndarray):
        ...
        if self._image_embedding is None:
            ...
            self._thread.start()
    ## Compute img embedding an cache them.
    ## Cache size: 10 entries (imgs)
    ## Chache is orderedDict
    ...
    def _get_image_embedding(self):
        if self._thread is not None:
            self._thread.join() ## Block execution until finish embedding computing.
            self._thread = None
        with self._lock:
